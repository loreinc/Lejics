function parser(lcode, dict) {
	...
	//console.log(result);
						...
						if (Number(numbt)) {
							cf(Number(numbt.replace(/&%5/gim, "+").replace(/&%6/gim, "-").replace(/&%7/gim, "*").replace(/&%8/gim, "/").replace(/&%s0/gim, " ")));
						//dict['ms'][fname.split("[")[0]]['type'] = 'number';
						continue;
						} 
					}
...
let cpath = fs.readFileSync("cfg/curretPathClean.ljcfg");
					...
					if (fname.split("(")[0] === "file.readall") {
							...
							let content = fs.readFileSync(path);
								...
								try {
								let content = fs.readFileSync(cpath+path);
		...
		Object.assign(scode, { 
								...
								"value" : {
									"type" : "string",
									"value" : '"' + content.replace(/"/gim, '&%2').replace(/\+/g, '&%5').replace(/\-/g, '&%6').replace(/\*/g, '&%7').replace(/\//g, '&%8') + '"'
								}});
		 lcode1 = lcode.slice(0,i+1);
								lcode2 = lcode.slice(i+1);
								lcode1.push(scode);
				lcode = lcode1.concat(lcode2);
			//	console.log(lcode);
