...
//});
    ...
    return match.replace(/&%sk1c/g, ')');
			...
			if (lsline.substring(0,2).toLowerCase() === "if" || lsline.substring(0,5).toLowerCase() === "while" || lsline.substring(0,3).toLowerCase() === "for") {
				...
				if (lsline.split("&:&il").length === 1 && lsline.substring(0,7).toLowerCase() !== "addfunc" && lsline.substring(0,8).toLowerCase() !== "emulate(") {
				lssort = lsline.split(":");
			}
			if (lsline.substring(0,7).toLowerCase() === "addfunc") {
				while(lsline.replace(/{/gim,"").length !== lsline.replace(/}/gim,"").length) {
					//console.log(unsorted);
					lsline += ", "+ lowsorted[i+1];
				...
				lssort = ["addfunc", lsline.replace("addfunc ","").replace(/,/gim,", ").trim()];
				//console.log(lssort);
